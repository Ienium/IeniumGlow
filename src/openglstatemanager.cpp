...
namespace ienium::glow
{
    ...
    void OpenGLStateManager::SetViewport(int pos_x, int pos_y, int width, int height)
    {
        if (pos_x != currentViewportX || pos_y != currentViewportY || width != currentViewportWidth || height != currentViewportHeight)
        {
            currentViewportX      = pos_x;
            currentViewportY      = pos_y;
