...
namespace ienium
{
    GLuint loadShaders(const std::filesystem::path& vertex_file_path, const std::filesystem::path& fragment_file_path)
    {
        ...
        if (vertex_shader_stream.is_open())
        {
            ...
            s_stream << vertex_shader_stream.rdbuf();
            vertex_shader_code = s_stream.str();
        ...
        {
            std::stringstream s_stream;
            s_stream << fragment_shader_stream.rdbuf();
            fragment_shader_code = s_stream.str();
        ...
        glShaderSource(vertex_shader_id, 1, &vertex_code_ptr, nullptr);
        ...
        glShaderSource(fragment_shader_id, 1, &fragment_code_ptr, nullptr);
