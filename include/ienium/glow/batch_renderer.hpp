...
namespace ienium::glow
{
    ...
    class BatchingSystem
    {
      public:
        ...
        void AddSpriteRenderCommand(RenderBatch& batch, const Vector2& size, const Vector2& position, ResourceId texture_id, const utils::Color& tint,
                                    const Vector2& texture_scale, const Vector2& texture_offset, float angle);
        ...
        void AddRenderCommand(RenderBatch& batch, Args&&... args)
        {
            ...
            if (batch.memoryChunksVBO.empty() || memoryManager->GetMemoryChunk(batch.memoryChunksVBO.back())->usedSize + ELEMENT_SIZE_MAP.at(SPRITE) >
                                                         memoryManager->GetMemoryChunk(batch.memoryChunksVBO.back())->size)
            {
      ...
      private:
        ...
        std::mutex                                        processBatchMutex;
        std::unordered_map<size_t, RenderBatch>::iterator nextBatchToProcess;

        // Batchwise functions
        void ProcessBatches();
        void ProcessBatch(RenderBatch& batch);
        void DrawBatches();
        ...
        // Helper functions
        static size_t CalculateHash(int layer, ResourceId texture_id, RenderType type);
    };
